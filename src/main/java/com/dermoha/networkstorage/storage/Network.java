...
public class Network {
    ...
    private Map<NetworkItemKey, Integer> cachedNetworkItems;
    ...
    public Network(String name, UUID owner) {
        ...
        this.cachedNetworkItems = new ConcurrentHashMap<>();
    ...
    public PlayerStat getPlayerStat(Player player) {
        return playerStats.computeIfAbsent(player.getUniqueId(),
                k -> new PlayerStat(player.getUniqueId(), player.getName()));
    }
    ...
    public boolean canAccess(Player player) {
        ...
        return playerUUID.equals(this.owner) || trustedPlayers.contains(playerUUID)
                || player.hasPermission("networkstorage.admin");
    }
    ...
    public Map<ItemStack, Integer> getNetworkItems() {
        ...
        try {
            ...
            for (Map.Entry<NetworkItemKey, Integer> entry : cachedNetworkItems.entrySet()) {
        ...
        try {
            ...
            for (Location chestLoc : allChestLocations) {
                if (chestLoc.getWorld() != null
                        && chestLoc.getWorld().isChunkLoaded(chestLoc.getBlockX() >> 4, chestLoc.getBlockZ() >> 4)) {
                    if (chestLoc.getBlock().getState() instanceof Chest) {
                        Chest chest = (Chest) chestLoc.getBlock().getState();
                        for (ItemStack item : chest.getInventory().getContents()) {
                            if (item != null && item.getType() != Material.AIR) {
                                NetworkItemKey key = new NetworkItemKey(item);
    ...
    private int getAmountInCache(ItemStack item) {
        return cachedNetworkItems.getOrDefault(new NetworkItemKey(item), 0);
    }
    ...
    public ItemStack removeFromNetwork(ItemStack itemToRemove, int amount) {
        ...
        try {
            ...
            for (Location chestLoc : allChestLocations) {
                if (amountRemoved >= amountToActuallyRemove)
                    break;
